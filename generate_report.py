...
def fill_section_6(doc, p):
    ...
    p6 = insert_p_after(p5, "在问题二中，我们以最小费用最大流模型进行优化分配。通过 Gurobi 精确求解，在受区间通过能力和车站捎带处理限制的双重约束空间内，系统以 100.0% 的完美比例完全满足了全网 1908.4 吨/日的全部运输需求，没有造成任何吨位的货流积压。此时，系统每日的最小总运营及装卸变动费用为 6,453,039.37 元/日（约 645.3 万元/日）。")
    format_paragraph(p6)
    p7 = insert_p_after(p6, "全网快运流量的运力承担模式产生了深刻的分化：货运动车组模式由于单列装载量大（85吨/列），构成了骨干运输格局，承运了 1212.6 吨/日的货运量（分担率 63.5%）；载客动车组及确认车捎带运输由于单站 48 吨的严格瓶颈，主要扮演了短途、零星、灵活的补白角色，承运了剩下的 695.8 吨/日货运量（分担率 36.5%）。平均吨公里运输成本约 3381 元/吨，吨均毛利约 420 元/吨。")
    format_paragraph(p7)
    ...
    tbl2_data = [
        ...
        ["其中：货运动车组承担量", "1212.6", "吨/日", "63.5% (物流骨干)"],
        ["其中：载客列车捎带承担量", "695.8", "吨/日", "36.5% (灵活补充)"]
    ]
    ...
    p9 = insert_p_after(p8, "在问题三中，我们引入了 12 个候选站点的建库决策和 4 种级别规模选择的 0-1 变量，建立了以系统整体商业净收益最大化为优化目标的混合整数规划模型。由 Gurobi 求解器在数秒内求出了全局最优的协同规划决策方案。")
    format_paragraph(p9)
    p10 = insert_p_after(p9, "最优基地选址与规模设计：在 12 个备选站点中，模型精准地选择了 7 个内陆枢纽城市建设快运物流基地，分别为武汉(WH)、长沙(CS)、南昌(NC)、郑州(ZZ)、重庆(CQ)、贵阳(GY)、成都(CD)。惊人的是，所有 7 个开放基地的建设级别均一致指向了投资最轻的第一级——「改建小规模」基地（规模编号 0，仅需 0.768 亿元/站）。东部走廊城市（上海 SH、杭州 HZ、南京 NJ、合肥 HF、徐州 XZ）则完全不予以建设——这些站点通过低成本、高密度的捎带模式即可高效覆盖。")
    format_paragraph(p10)
    p11 = insert_p_after(p10, "最优规划下系统的核心日均经济与实操指标如下：")
    format_paragraph(p11)
    p11_1 = insert_p_after(p11, "• 每日最大纯净收益：2,158,940.47 元/日 (约合 215.9 万元/日)；")
    format_paragraph(p11_1, first_line_indent=Pt(12))
    p11_2 = insert_p_after(p11_1, "• 每日运输总收入：2,808,289.26 元/日 (约合 280.8 万元/日)；")
    format_paragraph(p11_2, first_line_indent=Pt(12))
    p11_3 = insert_p_after(p11_2, "• 每日均摊折旧及固定人工折旧成本：75,945.21 元/日；")
    format_paragraph(p11_3, first_line_indent=Pt(12))
    p11_4 = insert_p_after(p11_3, "• 总投资额：5.376 亿元（7站 × 0.768亿元/站）；")
    format_paragraph(p11_4, first_line_indent=Pt(12))
    p11_5 = insert_p_after(p11_4, "• 实际承运快运货流量：785.6 吨/日 (约占总需求的 41.2%)；")
    format_paragraph(p11_5, first_line_indent=Pt(12))
    p11_6 = insert_p_after(p11_5, "• 货运动车组承运 95.8 吨/日（12.2%），捎带列车承运 689.8 吨/日（87.8%）；")
    format_paragraph(p11_6, first_line_indent=Pt(12))
    p11_7 = insert_p_after(p11_6, "• 全网货流量满足率：41.2%，剩余 1122.8 吨/日的低收益长尾货流被模型战略性放弃。")
    format_paragraph(p11_7, first_line_indent=Pt(12))

    p12 = insert_p_after(p11_7, "深度学术分析：为什么模型仅仅满足了 41.2% 的货流量，并且完全放弃了东部走廊的基地建设？这正是运筹优化在实际工程投资中极其耀眼的理性决策价值所在。")
    format_paragraph(p12)
    p13 = insert_p_after(p12, "第一，捎带模式的成本优势碾压了货运动车组。捎带列车的边际装卸成本仅为 120 元/吨，且不产生列车运行费和基地折旧费。相比之下，货运动车组的全成本（固定费+变动费+装卸费）在中短距离上也高达 2000-3500 元/吨。在捎带车站每日 48 吨的处理能力范围内（全路网 15 站 × 48 吨 = 720 吨/日），模型极度理性地将绝大部分货流（87.8%）分配给低成本捎带模式，仅在内陆枢纽间（如武汉-长沙-贵阳-成都）因捎带能力饱和或距离过长时，才启用货运动车组承运。")
    format_paragraph(p13)
    p14 = insert_p_after(p13, "第二，「改建小规模」的投资回报率碾压了新建方案。根据表 4-1 的参数分析，从「改建小规模」到「新建小规模」，每日摊销成本从 1.08 万元暴增至 11.1 万元（增长 10 倍），而吞吐能力仅提升 4 倍（1列→4列）。在当前年度的快运需求密度下，模型果断拒绝了全部新建方案，仅以最小的轻资产代价（5.376 亿元）撬动了最丰厚的利润回报。这种「轻资产、高收益、重干线」的精明决策，是单纯依靠专家经验所无法触及的运筹优化巅峰。")
    format_paragraph(p14)
    p15 = insert_p_after(p14, "第三，东部走廊的「零基地」策略深刻揭示了网络效应的经济学本质。上海、杭州、南京等东部枢纽城市拥有极高密度的捎带列车频次（载客动车组停站频繁），48 吨/日/站的捎带能力足以覆盖这些站点主要的高价值 OD 货流。在这些站点投入数亿元的基地建设费用，其边际收益远低于将节省的资本用于其他投资。模型以定量化的方式验证了「捎带优先、基地补充」这一双模协同策略的卓越经济性。")
    format_paragraph(p15)
    ...
    tbl3_data = [
        ["武汉 (WH)", "开放建设", "改建小规模", "30.0 (装卸) + 137 (通过)", "100.0% (饱和)"],
        ["长沙 (CS)", "开放建设", "改建小规模", "28.0 (装卸) + 142 (通过)", "100.0% (饱和)"],
        ["南昌 (NC)", "开放建设", "改建小规模", "13.0 (装卸) + 180 (通过)", "100.0% (饱和)"],
        ["郑州 (ZZ)", "开放建设", "改建小规模", "25.0 (装卸) + 150 (通过)", "100.0% (饱和)"],
        ["重庆 (CQ)", "开放建设", "改建小规模", "12.0 (装卸) + 183 (通过)", "100.0% (饱和)"],
        ["贵阳 (GY)", "开放建设", "改建小规模", "41.0 (装卸) + 109 (通过)", "100.0% (饱和)"],
        ["成都 (CD)", "开放建设", "改建小规模", "42.0 (装卸) + 107 (通过)", "100.0% (饱和)"],
        ["上海 (SH)", "关闭不建", "-", "0.0", "-"],
        ["杭州 (HZ)", "关闭不建", "-", "0.0", "-"],
        ["南京 (NJ)", "关闭不建", "-", "0.0", "-"],
        ["合肥 (HF)", "关闭不建", "-", "0.0", "-"],
    ...
    emu_routes_data = [
        ["长沙 (CS) ⇄ 贵阳 (GY) ⇄ 成都 (CD) ⇄ 重庆 (CQ)", "2715 km", "6.5 吨", "每日开行 1.0 对"],
        ["长沙 (CS) ⇄ 贵阳 (GY) ⇄ 成都 (CD)", "1354 km", "7.7 吨", "每日开行 1.0 对"],
        ["长沙 (CS) ⇄ 贵阳 (GY) ⇄ 重庆 (CQ) ⇄ 郑州 (ZZ)", "2120 km", "2.8 吨", "每日开行 1.0 对"],
        ["武汉 (WH) ⇄ 长沙 (CS)", "362 km", "12.3 吨", "每日开行 1.0 对"]
    ]
    for r_idx, row_data in enumerate(emu_routes_data):
        for c_idx, val in enumerate(row_data):
            populate_cell(tbl5.rows[r_idx+1].cells[c_idx], val)
            
    p19 = insert_p_after_element(tbl5._tbl, doc, "分析表 6-2 与 6-3 可以看出，货运动车组的开行路径高度集中于内陆枢纽之间（长沙⇄贵阳⇄成都⇄重庆⇄郑州⇄武汉），形成了中西部「品字形」快运走廊。东部沿海城市（上海、杭州、南京等）完全依赖捎带模式，实现了「捎带为主、货动为辅」的双模协同格局。每条 EMU 班列的日流量虽然不高（2-13 吨），但由于采用了 85 吨大列，其边际满载率仍有较大提升空间，这为未来需求增长后的「增频填舱」预留了弹性。")
    ...
    p23 = insert_p_after(p22, f"日均财务收支：在目前的 4.5元/吨公里 (货动) 与 3.0元/吨公里 (捎带) 计费率下，全网总运量每日可创造约 280.8 万元的总运输收入（其中捎带收入占 82.5%，EMU 收入占 17.5%）；扣除每日建设折旧(7.6万)、基地变动操作费(5.3万)、列车运行区间费(33.1万)、起讫装卸费(19.5万)和中转费等共约 65.5 万元后，系统每日可攫取纯利润 {daily_profit/1e4:.1f} 万元。")
    format_paragraph(p23)
    p24 = insert_p_after(p23, f"盈亏平衡回收期：系统的年平均净现金流入为 {annual_profit:.2f} 亿元/年。项目盈亏平衡的投资回收期为：{inv:.3f} 亿元 / {annual_profit:.2f} 亿元/年 = {payback_m/12:.2f} 年（约 {payback_m:.1f} 个月）。投资回报率 ROI = {p5_results.get('roi_annual', 148):.1f}%/年。在随后的 20 年特许经营周期内，该网络将为铁路部门累计创造高达 {npv_val:.1f} 亿元的累计净现值收益（折现率5%），内部收益率 IRR = {irr_val:.1f}%。")
    format_paragraph(p24)
    
    p24b = insert_p_after(p24, f"盈亏平衡分析表明：日固定成本约 7.6 万元（折旧+人工），平均吨收入约 3562 元/吨，边际贡献高达 2832 元/吨。因此只需每日承运约 {bep_tons:.0f} 吨货物（仅占总需求的 {bep_ratio:.1f}%）即可达到盈亏平衡点，远低于当前优化的 785.6 吨/日实际承运量，安全边际高达 96.6%。")
    format_paragraph(p24b)
    
    p24c = insert_p_after(p24b, "敏感性分析进一步确认：即使在需求下降 40% 的极端情景下，年净利润仍可保持 4.63 亿元；运价费率下调 30% 时年利润仍维持 7.36 亿元。建设成本上升 30% 仅使回收期从约 8.2 个月延长至约 10.7 个月。项目整体具有极强的抗风险弹性和财务健康度。")
    ...
    p26 = insert_p_after(p25, f"我们对高铁快运与其主要市场竞争对手——航空货运和公路干线卡车运输进行了系统量化对比分析（详见 problem6.py）。以 200 吨货物的运输场景为例，纯运输成本的对比表明：公路卡车在 0-5000km 全距离范围内具有绝对的成本优势（约为高铁的 1/4 至 1/3），高铁快运的固定成本（50000元/列）远高于公路（2000元/车），但高铁在时效性（250km/h vs 80km/h）、准点率（99% vs 85%）和环保性（碳排放仅为公路的 1/5）方面具有压倒性优势。航空货运虽然速度最快（800km/h），但其高昂的固定成本（80000元/架次）和装卸费（500元/吨）使其在 0-5000km 范围内总成本始终高于高铁。")
    ...
    p27 = insert_p_after(curr_p2, "基于上述市场博弈格局，高铁快运在 500-1500 km 距离带上具有显著的「时效-成本-可靠性」综合竞争优势（纯运输成本公路最低，但含时间价值后高铁在中长距离最优）。如果希望进一步拓展盈利版图，我们提出三大战略改善建议：")
